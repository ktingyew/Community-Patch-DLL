...
fine; there's no need to preserve upstream conventions for their own sake. 

Every 'feature commit' should be one atomic, self-contained tweak (one balance change,
one bugfix, one feature), so any of them can be isolated, dropped, or modified
later without disturbing the others. Such commit message should be prefixed with
`kty-feat: `. You should see examples in the later branches (e.g. `my-5.4.6-build-GEMS`).
...
Each `my-<version>-build[-<feature>]` branch is a snapshot forked from an upstream
`Release-<version>` tag, carrying the author's personal commits on top (custom UA tweaks,
balance changes, build tooling, etc). Several such branches may exist for the same upstream
version, each carrying a different feature set (e.g. `my-5.4.6-build-GEMS`,
`my-5.4.6-build-oneGPperCity`).

**The lead branch is whatever branch is currently checked out** (`git branch --show-current`).
There is no hardcoded pointer to maintain. Assume the user means the checked-out branch
unless they explicitly name another one.

Heuristic if you ever need to guess (e.g. nothing relevant is checked out): the lead branch
is almost certainly one of the `my-*-build*` branches with the HIGHEST upstream version
number, compared as major.minor.patch (`5.4.6` > `5.4.4` > `5.2.7`). Branches from older
versions (e.g. `my-5.2.7-build`, `my-5.4.4-build`) are frozen historical snapshots — do not
try to propagate new changes to them; that's intentional to minimise work done, not an
oversight.

**Moving a branch forward** (when the user wants to adopt a newer upstream release):
1. `git fetch upstream` (remote `upstream` = `LoneGazebo/Community-Patch-DLL`), confirm the
   target `Release-<version>` tag exists.
2. `git checkout -b my-<new-version>-build[-<feature>] Release-<new-version>`
3. List the personal commits on the old branch since its own base tag
   (`git log --oneline --reverse Release-<old-version>..my-<old-version>-build[-<feature>]`),
   then `git cherry-pick` them in order onto the new branch. Even if `CvPlayer.cpp` etc. have
   ...
   A conflict in the generated `.modinfo` is almost always just stale MD5 hashes: take
   upstream's side, then regenerate it (see "Personal SQL/XML overrides" below).

This is a copy, not a destructive rewrite — the old branch is left untouched and becomes
one of the frozen historical branches.
